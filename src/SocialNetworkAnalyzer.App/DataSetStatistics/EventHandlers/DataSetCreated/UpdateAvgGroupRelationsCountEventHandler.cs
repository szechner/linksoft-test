...
/// <summary>
/// <see cref="DataSetCreatedEvent"/> handler to update the average groups relations count in the data set statistics
/// </summary>
public sealed class UpdateAvgGroupRelationsCountEventHandler(IServiceScopeFactory serviceScopeFactory) : IEventHandler<DataSetCreatedEvent>
{
    public async Task Handle(DataSetCreatedEvent dataSetCreatedEvent, CancellationToken cancellationToken)
    {
        using var scope = serviceScopeFactory.CreateScope();
        var serviceProvider = scope.ServiceProvider;
        var logger = serviceProvider.GetRequiredService<ILogger<UpdateAvgGroupRelationsCountEventHandler>>();
        ...
        try
        {
            var avgRelationsCount = await relationshipRepository.GetAvgGroupRelationsCount(dataSetCreatedEvent.DataSetId, cancellationToken);
            await dataSetStatisticsRepository.UpdateAvgGroupRelationsCount(dataSetCreatedEvent.DataSetId, avgRelationsCount, cancellationToken);
        }
        catch (Exception e)
        {
